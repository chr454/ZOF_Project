...
# Reuse solver functions (same logic as CLI but adapted to return data lists)
...
def modified_secant(f, x0, delta, max_iter, tol):
    ...
    for i in range(1, max_iter+1):
        f_x0 = f(x0)
        denom = f(x0 + delta * x0) - f_x0
        if denom == 0:
            raise ValueError("Denominator zero in Modified Secant.")
        x1 = x0 - (delta * x0 * f_x0) / denom
        err = abs(x1 - x0)
        rows.append((i, x0, f_x0, x1, err))
        if err < tol:
            return x1, abs(f(x1)), i, rows
        x0 = x1
    return x1, abs(f(x1)), max_iter, rows
...
def index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = f"Invalid function expression: {e}"
            return render_template('index.html', error=error)

        try:
            if method == 'bisection':
                a = float(request.form.get('a')); b = float(request.form.get('b'))
                root, final_err, iters, rows = bisection(f, a, b, max_iter, tol)
                result = {'method': 'Bisection', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            elif method == 'regula':
                a = float(request.form.get('a')); b = float(request.form.get('b'))
                root, final_err, iters, rows = regula_falsi(f, a, b, max_iter, tol)
                result = {'method': 'Regula Falsi', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            elif method == 'secant':
                x0 = float(request.form.get('x0')); x1 = float(request.form.get('x1'))
                root, final_err, iters, rows = secant(f, x0, x1, max_iter, tol)
                result = {'method': 'Secant', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            elif method == 'newton':
                x0 = float(request.form.get('x0'))
                root, final_err, iters, rows = newton_raphson(f, df, x0, max_iter, tol)
                result = {'method': 'Newton-Raphson', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            elif method == 'fixed':
                ...
                result = {'method': 'Fixed Point', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            elif method == 'modified_secant':
                x0 = float(request.form.get('x0')); delta = float(request.form.get('delta'))
                root, final_err, iters, rows = modified_secant(f, x0, delta, max_iter, tol)
                result = {'method': 'Modified Secant', 'root': root, 'final_err': final_err, 'iters': iters, 'rows': rows}
            else:
